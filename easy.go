...
func (curl *CURL) Perform() (error, int) {
	p := curl.handle
	errno := C.curl_easy_perform(p)
	// Safe conversion from uint32 to int, considering cURL error ranges in between 0 - 83
	return newCurlError(errno), int(uint32(C.uint(errno)))
}
